...
const Booking = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    data.append('form-name', 'booking'); // MATCHES HIDDEN FORM IN INDEX.HTML
  ...
  if (status === 'success') {
    return (
      <div className="min-h-screen pt-20 px-4 bg-slate-950 text-white flex items-center justify-center">
        <div className="max-w-md w-full bg-slate-900/50 p-8 rounded-2xl border border-purple-500/30 backdrop-blur-sm text-center animate-fade-in-up">
          <div className="w-16 h-16 bg-purple-500/10 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle className="w-8 h-8 text-purple-400" />
          </div>
          <h2 className="text-2xl font-bold text-white mb-4">Request Received!</h2>
          <p className="text-slate-400 mb-8">
            Your booking request has been securely transmitted. We will review the time and send a calendar invitation to your email shortly.
          </p>
          <button 
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:border-purple-400 focus:ring-1 focus:ring-purple-400 focus:outline-none transition-all"
                  />
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  value={formData.date}
                  onChange={handleChange}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-white focus:border-purple-400 focus:ring-1 focus:ring-purple-400 focus:outline-none transition-all [color-scheme:dark]"
                />
              </div>
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-purple-500 to-pink-600 text-white font-bold py-4 rounded-lg hover:shadow-lg hover:shadow-purple-500/20 transition-all flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {status === 'submitting' ? (
                  <>
